...
function AboutMEO() {
  ...
  const DynamicTokenModel = ({ width }: { width?: string }) => (
    ...
    >
      ...
      <Text color={theme.text} marginTop="24px" textAlign={above500 ? 'start' : 'center'} lineHeight={1.5}>
        <Trans>
          MEO enables MetabeoDAO to shape token behaviour and upgrades, making MEO much more adaptable and providing
  ...
  const ParticipationRewards = ({ width }: { width?: string }) => (
    ...
    >
      ...
      <Text color={theme.text} marginTop="24px" textAlign={above500 ? 'start' : 'center'} lineHeight={1.5}>
        <Trans>
          MEO holders can stake MEO in MetabeoDAO and vote on important decisions. Voters receive trading fees generated
          on MetabeoSwap and other benefits from ecosystem collaborations on Metabeo.
        </Trans>
      </Text>
    </ForLiquidityProviderItem>
  )
  ...
  const LiquidityIncentitives = ({ width }: { width?: string }) => (
    ...
    >
      ...
      <Text color={theme.text} marginTop="24px" textAlign={above500 ? 'start' : 'center'} lineHeight={1.5}>
        <Trans>
          MetabeoDAO can propose various MEO incentives like liquidity mining rewards on MetabeoSwap on top of standard
    ...
    >
      <AboutPage>
        <Wrapper>
          ...
          >
            <Trans>
              MEO is a utility and governance token and an integral part of Metabeo Network and its product MetabeoSwap
          ...
          <Text as="h2" marginTop="12px" fontWeight="500" fontSize={['28px', '36px']} textAlign="center">
            <Trans>What is MEO used for?</Trans>
          </Text>
          <Text color={theme.text} marginTop={['40px', '48px']} fontSize="1rem" textAlign="center" lineHeight={1.5}>
            <Trans>
              MEO token holders can benefit from our flagship product MetabeoSwap. Holders can stake their MEO & vote on
              initiatives to receive trading fees generated on MetabeoSwap! More trades on MetabeoSwap can generate more
              rewards for MEO holders!
              <br />
              <br />
              MEO token is dynamic - it can be upgraded, minted or burned by MetabeoDAO to better support liquidity and
              growth.
            </Trans>
          </Text>
          ...
          >
            ...
            <Flex width="100%" flexDirection="column" height="max-content">
              <Text fontSize={['20px', '24px']} fontWeight={500} color={theme.primary}>
                <Trans>METABEO DAO</Trans>
              </Text>
              <Text as="h2" marginTop="12px" fontWeight="500" fontSize={['28px', '36px']}>
                <Trans>Stake MEO, Vote, Earn Rewards.</Trans>
              </Text>
              <Text fontSize="16px" marginTop={['40px', '48px']} color={theme.text} lineHeight="24px" textAlign="left">
                <Trans>
                  MetabeoDAO is a community platform that allows MEO token holders to participate in governance. MEO
                  holders can stake MEO to vote on proposals. In return, they receive rewards from fees generated on
                  MetabeoSwap through trading activities in Metabeo Network.
                </Trans>
              </Text>
              <img
                width="100%"
              ...
              />
              <ExternalLink href="https://metabeo.net">
                <Flex
                  width={above768 ? '236px' : '100%'}
                  marginTop={['40px', '48px']}
                  sx={{ gap: above768 ? '24px' : '16px' }}
          ...
          <Text
            ...
            textAlign="center"
          >
            <Trans>Where you can buy MEO</Trans>
          </Text>
          ...
          <Text
            ...
            textAlign="center"
          >
            <Trans>Where you can store MEO</Trans>
          </Text>
          <Text fontSize="16px" marginTop={['40px', '48px']} color={theme.text} lineHeight="24px" textAlign="center">
            <Trans>
              MEO is an ERC-20 token, so it can be stored in many Web3 wallets you control. Below are some examples.
            </Trans>
          </Text>
          ...
          <MoreInfoWrapper>
            ...
            >
              <Text as="h2" fontWeight="500" fontSize={['28px', '36px']}>
                <Trans>More information about MEO is available on:</Trans>
              </Text>
              <Flex flexDirection={above768 ? 'row' : 'column'} marginTop="48px" style={{ gap: '48px' }}>
                <ExternalLink href={`https://www.coingecko.com/`}>
      ...
      <Footer background={isDarkMode ? theme.background : theme.white}>
        <FooterContainer>
          <Flex flexWrap="wrap" sx={{ gap: '12px' }} justifyContent="center">
            <ExternalLink href={`https://docs.metabeo.com`}>
              <Trans>Docs</Trans>
            </ExternalLink>
            <VerticalDivider />
            <ExternalLink href={`https://github.com/metabeo`}>
              <Trans>Github</Trans>
            </ExternalLink>
            <VerticalDivider />
            <ExternalLink href={`https://metabeo.net`}>MetabeoDAO</ExternalLink>
            <VerticalDivider />
            <ExternalLink href={`https://metabeo.net`}>
              <Trans>Forum</Trans>
            </ExternalLink>
            {!above500 ? <div /> : <VerticalDivider />}
            <ExternalLink href={`https://metabeo.network`}>MEO Network</ExternalLink>
            <VerticalDivider />
            <StyledInternalLink to={`/about/meo`}>MEO</StyledInternalLink>
          </Flex>
          <FooterSocialLink />
        </FooterContainer>
      </Footer>
